...
Graph2.prototype.updatePeriod = function() {
	...
	this.bioData.push(this.antData[this.antData.length-1]+this.larvaData[this.larvaData.length-1]);
...
Graph2.prototype.drawPeriod = function(ctx) {
	...
	if (this.bioData.length > 1) {
		//entire pop(larva + ant)
		this.ctx.strokeStyle = "#00BB00";
		this.ctx.lineWidth = 2;

		this.ctx.beginPath();
		var xPos = this.x;
		var yPos = this.mound.tick > TICK_DISPLAY ? this.y+this.ySize-Math.floor(this.bioData[this.mound.tick-TICK_DISPLAY]/this.maxVal*this.ySize)
										: this.y+this.ySize-Math.floor(this.bioData[0]/this.maxVal*this.ySize);
		this.ctx.moveTo(xPos, yPos);
		var length = this.mound.tick > TICK_DISPLAY ?
					 TICK_DISPLAY : this.bioData.length;
		for (var i = 1; i < length; i++) {
			var index = this.mound.tick > TICK_DISPLAY ?
						this.mound.tick-TICK_DISPLAY-1+i : i;
			xPos++;
			yPos = this.y+this.ySize-Math.floor(this.bioData[index]/this.maxVal*this.ySize);
			if (yPos <= 0) {
				yPos = 0;
			}
		...
		this.ctx.fillText(this.bioData[this.bioData.length-1], this.x+this.xSize+15, yPos+10);
		...
		this.ctx.strokeStyle = "#000000";
		this.ctx.fillSytle = "#000000";
		this.ctx.fillText(this.foodData[this.foodData.length-1], this.x+this.xSize+15, yPos+10);
...
Graph2.prototype.updateMax = function() {
	...
	if (tick > TICK_DISPLAY) {
		var recentBio = this.bioData.slice(tick-TICK_DISPLAY);
		var recentFood = this.foodData.slice(tick-TICK_DISPLAY);
		
		this.maxVal = Math.max(...recentBio,
							   ...recentFood);
	} else {
		this.maxVal = Math.max(...this.bioData, 
							   ...this.foodData);
	}
}